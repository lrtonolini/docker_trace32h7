...
void T32_Term_PutBlock(const unsigned char * data, int block_size) {
	...
	while (block_size > 0) {
		...
		len = block_size;
		if (len + package_write_index > (T32_Term_Tar2HostBufferSize-sizeof(long)))
		...
		if (package_write_index >= package_size || package_write_index >= (T32_Term_Tar2HostBufferSize-sizeof(long))) {
			/* finish package */
