...
; @Description: ThreadDebug dual serial port demo for TRACE32 GDB FrontEnd debugger
...
This demo program is based on the Arduino ThreadDebug example. It include the MRI serial monitor for debugging with TRACE32 front-end for GDB, via serial port. In addition a second serial port over USB has been defined for printing, and TRACE32 Term command added to show print output.

The main program T32ThreadDebugPrint.ino define SerialUSB1 as the serial port dedicated to GDB debugger and SerialUSB as the serial port for printing.

The start script also open the TRACE32's Term window to show application print output. 

Note: in case you have a build error with SerialUSB1, then you need to update your Arduino IDE or add the latest Arduino Core libraries for Portenta H7.
...
The demo application is compiled to break at setup() and wait for the debugger attach, and in fact LEDs are not blinking. This is due to this definition in T32ThreadDebugPrint.ino:
...
Please refer to the manual /pdf/training_debugger.pdf to learn about TRACE32 debug functions.
